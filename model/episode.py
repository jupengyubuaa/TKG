...
class Episode(nn.Module):
    ...
    def forward(self, query_entities, query_timestamps, query_relations):
        ...
        return all_loss, all_logits, all_actions_idx, current_entities, current_timestamps
    ...
    def beam_search(self, query_entities, query_timestamps, query_relations):
        ...
        current_entities = torch.gather(action_space[:, :, 1], dim=1, index=top_k_action_id).reshape(-1)  # [batch_size * beam_size]
        ...
        for t in range(1, self.path_length):
            ...
            Hq_roll=Hq.repeat(1,1,beam_size)                                                                    #计算得分二
            Hq_roll=Hq_roll.reshape([batch_size*beam_size,-1])                                                  #计算得分二
            ########################################beam_size 其实内容重复的行#########################################################################
            action_space = self.env.next_actions(current_entities, current_timestamps,
            ...
            offset = offset.unsqueeze(-1).repeat(1, 1, self.config['state_dim'])  # [batch_size, beam_size]  不应该是[batch_size,beam_size,state_dim]???
            ...
            current_entities = torch.gather(action_space[:, :, 1].reshape(batch_size, -1), dim=1, index=top_k_action_id).reshape(-1)
