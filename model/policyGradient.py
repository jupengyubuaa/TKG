...
class PG(object):
    ...
    def get_reward(self, current_entites, answers):
         # positive = torch.ones_like(current_entites, dtype=torch.float32) * self.positive_reward+pathscore[-1].squeeze()
    ...
    def entropy_reg_loss(self, all_logits):
        all_logits = torch.stack(all_logits, dim=2) #[batch_size,action,3]
        #输出维度
        #########################all_logits怎么来的#################################################
    ...
    def calc_reinforce_loss(self, all_loss, all_logits, cum_discounted_reward):
        loss = torch.stack(all_loss, dim=1) #[batch_size,3]
        base_value = self.baseline.get_baseline_value()
        final_reward = cum_discounted_reward - base_value #[batch_size,3]
        reward_mean = torch.mean(final_reward)
        reward_std = torch.std(final_reward) + 1e-6
        final_reward = torch.div(final_reward - reward_mean, reward_std)   #############标准正态分布形式##############
